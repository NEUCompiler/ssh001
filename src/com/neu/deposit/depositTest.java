...
public String DepositCount() {
		...
		Query query = session
				.createQuery("from com.neu.deposit.Deposit WHERE time = ? and sign=0");
		...
		if (deposit.isEmpty()) {
			...
			if(cn.getTime()<13){
				if(exectlytime<13){
					...
					interest=cn.getRate()*amount + cn1.getRate()*(exectlytime-12)*amount/12;
					revenue=interest*cn.getInterest();
					total=interest-revenue+amount;	
					average=total/exectlytime;
				}
				...
				if(exectlytime<25){
					...
					interest=cn.getRate()*amount*2 + cn1.getRate()*(exectlytime-24)*amount/12;
					revenue=interest*cn.getInterest();
					total=interest-revenue+amount;	
					average=total/exectlytime;
				}
				...
				if(exectlytime<37){
					...
					interest=cn.getRate()*amount*3 + cn1.getRate()*(exectlytime-36)*amount/12;
					revenue=interest*cn.getInterest();
					total=interest-revenue+amount;	
					average=total/exectlytime;
				}	
